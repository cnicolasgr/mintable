    ...
    public accountSetup = (): Promise<void> => {
        return new Promise((resolve, reject) => {
            ...
            app.post('/create_link_token', async (req, res) => {
                ...
                const country_codes = process.env.COUNTRY_CODES ? process.env.COUNTRY_CODES.split(',') : ['FR']
                const language = process.env.LANGUAGE ? process.env.LANGUAGE : 'fr'
        ...
        return this.fetchPagedTransactions(accountConfig, startDate, endDate)
            .then(data => {
                let accounts: Account[] = data.accounts.map(account => ({
                    ...
                    mask: account.mask,
                    institution: accountConfig.institution ?? account.name,
                    account: accountConfig.name ?? account.official_name,
